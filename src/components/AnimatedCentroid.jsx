...
export default function AnimatedCentroid({ data }) {
  ...
  return (
    <div className="card shadow rounded-xl p-6 mb-10 mt-10">
        <h2 className="text-3xl font-bold mb-4 mt-4">Traiectoria centrului de poluare</h2>
        ...
        <p className="text-xl font-semibold mb-2">
            <strong>Data: </strong>{new Date(current.day).toLocaleDateString("ro-RO", {
                day: "numeric", month: "long", year: "numeric"
            })}
        </p>
        <p className="text-xl font-semibold mb-2">
            <strong>Locația: </strong> {current.name}
            {countyInfo && `, județul ${countyInfo.name}`}
        </p>
        <p className="text-xl font-semibold mb-2">
            <strong>Valoare: </strong>{Number(current.max_value).toExponential(3)}
        </p>
        ...
        >
            ...
            >
                <Popup>
                {point.name}<br />
                {new Date(point.day).toLocaleDateString("ro-RO")}<br />
                Valoare: {Number(point.value).toExponential(3)}
                </Popup>
            </Marker>
            ))}
